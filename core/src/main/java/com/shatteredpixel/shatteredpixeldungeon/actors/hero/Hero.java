...
public class Hero extends Char {
	...
	public int attackSkill( Char target ) {
		...
		if (!(wep instanceof MissileWeapon)) {
			...
			if (buff(Momentum.class) != null && buff(Momentum.class).freerunning()){
				accuracy *= 1f + pointsInTalent(Talent.PROJECTILE_MOMENTUM)/5f;
			}
		}
		...
		while (this.exp >= maxExp()) {
			...
			if (lvl < MAX_LEVEL) {
				...
				if (buff(RingOfMimic.Mimic.class) != null){
					buff(RingOfMimic.Mimic.class).onLevelUp();
				}
	...
	public void resurrect() {
		...
		for (Item i : belongings){
			...
			} else if (i instanceof RingOfMimic && i.keptThroughLostInventory() && hasTalent(Talent.LIGHT_GREED)) {
				((RingOfMimic) i).activate(this);
